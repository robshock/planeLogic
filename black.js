...
const BlackYLimit3 = window.innerHeight * 0.55;
const BlackYLimit4 = window.innerHeight * 0.44;

const BlackXLimit = window.innerWidth * 0.525;
const BlackXLimit2 = window.innerWidth * 0.62;
const BlackXLimit3 = window.innerWidth * 0.815;

let movingDownBlack = false;
let BlackState = "up"; // "up" → "right1" → "right2" → "down1" → "down2" → "down3" → "down4"
...
function animateBlack() {
   ...
   // ================= DOWN (segment 3) =================
   ...
   else if (BlackState === "down3") {
       BlackY -= speedBlack;
       if (BlackY <= BlackYLimit4) {
           if (!seventhDecisionMadeBlack && BlackY >= decisionPoint7) {
               seventhDecisionMadeBlack = true;
       ...
       BlackX += speedBlack;
       if (BlackX >= BlackXLimit3) {
           BlackState = "down4";
       }
   }

   else if (BlackState === "down4") {
       BlackY -= speedBlack;
   }
   ...
   if (BlackState === "down4" && BlackY < window.innerHeight * 0.25) {
